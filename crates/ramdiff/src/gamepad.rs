...
mod tests {
    ...
    fn xinput_buttons_map_to_pad_bits() {
        ...
        s.feed(&event(EV_KEY, 304, 1)); // A down
        s.feed(&event(EV_KEY, 315, 1)); // Start down
        assert_eq!(s.pad_bits(), (1 << 0) | (1 << 10));
        s.feed(&event(EV_KEY, 304, 0)); // A up
    ...
    fn dinput_buttons_map_to_pad_bits() {
        let mut s = PadState::default();
        s.feed(&event(EV_KEY, 288, 1)); // X (D-mode)
        s.feed(&event(EV_KEY, 296, 1)); // Back -> Select
    ...
    fn hat_maps_to_dpad_bits_and_releases() {
        let mut s = PadState::default();
        s.feed(&event(EV_ABS, ABS_HAT0X, -1));
        s.feed(&event(EV_ABS, ABS_HAT0Y, 1));
        assert_eq!(s.pad_bits(), (1 << 8) | (1 << 7)); // Left + Down
        s.feed(&event(EV_ABS, ABS_HAT0X, 0));
        s.feed(&event(EV_ABS, ABS_HAT0Y, 0));
    ...
    fn xpad_hat_full_range_values_still_signum() {
        ...
        let mut s = PadState::default();
        s.feed(&event(EV_ABS, ABS_HAT0X, 32767));
    ...
    fn auto_repeat_counts_as_held() {
        let mut s = PadState::default();
        s.feed(&event(EV_KEY, 305, 2)); // B auto-repeat
    ...
    fn unknown_codes_and_types_are_ignored() {
        let mut s = PadState::default();
        s.feed(&event(EV_KEY, 999, 1));
        s.feed(&event(0x04, 4, 589826)); // EV_MSC scan code noise
        s.feed(&event(EV_ABS, 0, -32768)); // left stick ignored by design
        assert_eq!(s.pad_bits(), 0);
    }
